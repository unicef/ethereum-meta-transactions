...
contract('GnosisSafe with refunds', function(accounts) {
    ...
    beforeEach(async function () {
        ...
        verifier = await utils.deployContract("deploying Verifier", Verifier)
        // Create Gnosis Safe
    ...
    it('should deposit and withdraw 1 ETH using meta transaction', async () => {
        // Deposit 1 ETH + some spare money for execution
        assert.equal(await web3.eth.getBalance(gnosisSafe.address), 0)
        ...
        let executorBalance = await web3.eth.getBalance(executor)
        let userBalance = await web3.eth.getBalance(lw.accounts[0])

        // Withdraw 1 ETH
        await safeUtils.executeMetaTransaction(verifier, lw, gnosisSafe, 'executeTransaction withdraw 0.5 ETH', [lw.accounts[0], lw.accounts[2]], accounts[0], web3.utils.toWei("0.5", 'ether'), "0x", CALL, executor)

        assert.notEqual(await web3.eth.getBalance(executor), executorBalance)
        assert.equal(userBalance, await web3.eth.getBalance(lw.accounts[0]))
