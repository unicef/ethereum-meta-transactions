...
contract Verifier {
    ...
    constructor() {
        ownerWhitelist[msg.sender] = true;
    }
    //to avoid replay
    ...
    function updateOwnerWhitelist(address _account, bool _value) public returns(bool) {
        require(ownerWhitelist[msg.sender],"Verifier::updateWhitelist Account Not Whitelisted");
        ownerWhitelist[_account] = _value;
        emit UpdateOwnerWhitelist(_account,_value);
        return true;
    }
    event UpdateOwnerWhitelist(address _account, bool _value);

    function updateCallWhitelist(address _account, bool _value) public returns(bool) {
        require(ownerWhitelist[msg.sender],"Verifier::updateWhitelist Account Not Whitelisted");
        callWhitelist[_account] = _value;
        emit UpdateCallWhitelist(_account,_value);
        return true;
    }
    event UpdateCallWhitelist(address _account, bool _value);
    ...
    function forward(bytes memory sig, address signer, address destination, uint value, bytes memory data) public {
        require(ownerWhitelist[msg.sender],"Verifier::forward Owner Account Not Whitelisted");
        ...
        require(MessageIsSigned(signer,_hash,sig),"Verifier::forward message is not signed correctly");
        // only whitelisted contract addresses are accepted
        require(callWhitelist[destination],"Verifier::forward Smart Contract Address Not Whitelisted");
        ...
        require(executeCall(destination, value, data), "Verifier::forward problem in executeCall");
